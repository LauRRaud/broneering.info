...
for(const [key,title,prices,times] of [
 ...
 lengths.forEach((length,index)=>add(`${key}-${index}`,['Juuksur'],`${title} — ${length.toLocaleLowerCase('et')}`,prices[index],times[index]));
}
...
] as const)add(key,['Juuksur'],name,price,duration);
for(const [key,name,price,duration] of [
...
for(const [key,title,install,maintenance] of [
 ['classic-lashes','Klassikaliste ripsmete',35,30],['hybrid-lashes','Hübriidripsmete',45,40],
] as const){
 add(`${key}-install`,['Ripsmed'],`${title} paigaldus`,install,120);
 add(`${key}-maintenance`,['Ripsmed'],`${title} hooldus`,maintenance,90);
}
add('pedicure-gel',['Küünehooldus'],'Spa-pediküür + geellakk',35,75);
add('pedicure',['Küünehooldus'],'Spa-pediküür geellakita',30,60);
