...
export type Catalog = { tenant: { name: string; slug: string; address: string; timezone: string; description: string; cancellationHours: number; rulesVersion:number; demo: boolean }; services: Service[]; staff: Staff[]; today: string; maxDate: string };
