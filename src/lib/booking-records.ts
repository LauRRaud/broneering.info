...
export function bookingSnapshot(row:BookingRow){
  return {...bookingResult(row),emailReminder:row.customer_reminders!==false,smsReminder:row.customer_sms_reminders===true,serviceId:row.service_id,staffId:row.staff_id,bufferBefore:row.buffer_before,bufferAfter:row.buffer_after,source:row.source,attentionReason:row.attention_reason};
}
...
export async function queueBookingNotice(client:PoolClient,row:BookingRow,kind:string){
  ...
  if(enabled&&row.customer_reminders!==false&&row.status==='confirmed'&&settings.reminder_minutes!=null){
