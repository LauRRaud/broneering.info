...
export const scheduleSchemas={
  ...
  'save-exception':z.object({...scope,...range,closed:z.boolean(),acknowledgeConflicts:z.boolean().default(false),intervals,kind:z.enum(['vacation','illness','extra_work','other'])}).strict().refine(validRange).refine(d=>!d.acknowledgeConflicts||d.closed).refine(d=>d.closed?d.intervals.length===0:d.intervals.length>0).refine(d=>!['vacation','illness'].includes(d.kind)||d.closed).refine(d=>d.kind!=='extra_work'||!d.closed),
