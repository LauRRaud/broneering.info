...
export const serviceManagementSchemas={
 ...
 'save-service':z.object({...identity,groupId:z.uuid(),name:z.string().trim().min(1).max(150),description:z.string().trim().max(1000),defaultPrice:price,defaultDuration:duration,bufferBefore:buffer,bufferAfter:buffer,active:z.boolean(),online:z.boolean()}).strict(),
...
export type ManagedService={id:string;groupId:string|null;name:string;description:string;defaultPrice:number;defaultDuration:number;bufferBefore:number;bufferAfter:number;active:boolean;online:boolean;version:number};
