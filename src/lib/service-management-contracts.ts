import {publicPhoneHref} from './public-phone';
...
export const serviceManagementSchemas={
 ...
 'save-staff':z.object({...identity,publicPhone:z.string().trim().max(30).refine(value=>!value||publicPhoneHref(value)!==null).optional(),name:z.string().trim().min(1).max(120),title:z.string().trim().max(150),bio:z.string().trim().max(1000),photoUrl:z.string().max(2000).refine(value=>{if(!value||/^\/api\/staff-photos\/[0-9a-f-]{36}\?v=\d+$/.test(value))return true;try{const url=new URL(value);return url.protocol==='https:'&&!url.username&&!url.password;}catch{return false;}}),active:z.boolean(),online:z.boolean()}).strict(),
...
export type ManagedStaff={publicPhone?:string;id:string;name:string;title:string;bio:string;photoUrl:string;active:boolean;online:boolean;version:number};
