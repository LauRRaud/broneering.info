...
export const colorLabels={background:'Lehe taust',backgroundMid:'Tausta kesktoon',backgroundEdge:'Tausta servatoon',surface:'Sisuala taust',text:'Põhitekst',heading:'Pealkirjad',link:'Lingid',border:'Piirjooned ja fookus',button:'Nupu taust',buttonText:'Nupu tekst',calendar:'Kalendri taust',calendarText:'Kalendri tekst',selected:'Valitud aja taust',selectedText:'Valitud aja tekst',icons:'Ikoonid',navigation:'Nooled ja navigeerimine',cardBorder:'Kaartide piirjooned',mutedText:'Abitekstid',cursor:'Kursori täide',cursorHover:'Kursori täide nupul',cursorOutline:'Kursori kontuur'} as const;
export type ColorKey=keyof typeof colorLabels;
const color=z.string().regex(/^#[0-9a-fA-F]{6}$/);
export const paletteSchema=z.object({background:color,backgroundMid:color.optional(),backgroundEdge:color.optional(),surface:color,text:color,heading:color,link:color,border:color,button:color,buttonText:color,calendar:color,calendarText:color,selected:color,selectedText:color,icons:color.optional(),navigation:color.optional(),cardBorder:color.optional(),mutedText:color.optional(),cursor:color.optional(),cursorHover:color.optional(),cursorOutline:color.optional()}).strict();
export type Palette=z.infer<typeof paletteSchema>;
export function resolvedPalette(p:Palette){return {...p,cursor:p.cursor??'#f1e8d9',cursorHover:p.cursorHover??'#f1ddb7',cursorOutline:p.cursorOutline??'#a59b8e',backgroundMid:p.backgroundMid??p.background,backgroundEdge:p.backgroundEdge??p.background,icons:p.icons??p.heading,navigation:p.navigation??p.heading,cardBorder:p.cardBorder??p.border,mutedText:p.mutedText??p.text};}
...
export function contrastIssues(config:ThemeConfig):ContrastIssue[]{
  ...
  for(const bg of ['background','backgroundMid','backgroundEdge','surface'] as const){for(const fg of ['text','heading','link'] as const)checks.push([fg,bg,4.5]);checks.push(['border',bg,3],['selected',bg,3]);}
  for(const bg of ['background','backgroundMid','backgroundEdge','surface'] as const){checks.push(['mutedText',bg,4.5],['navigation',bg,3],['icons',bg,3]);}
