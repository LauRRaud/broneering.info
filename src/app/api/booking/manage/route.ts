...
export async function GET(request:Request){
  try{
    const tenant=await tenantForHost(request.headers.get('host')??'');limitTenant(`management-read:${tenant.id}`,180);
  ...
  }catch(error){return protect(errorResponse(error));}
}
export async function POST(request:Request){
  try{
    assertSameOrigin(request);const tenant=await tenantForHost(request.headers.get('host')??'');limitTenant(`management-write:${tenant.id}`,60);
