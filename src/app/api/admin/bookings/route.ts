...
const queries=z.discriminatedUnion('view',[
  z.object({view:z.literal('list'),tenantId:z.uuid(),day:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),attention:z.enum(['1','0']).optional(),mode:z.enum(['list','day','week']).default('list'),staffId:z.uuid().optional(),page:z.coerce.number().int().min(0).max(10000).default(0)}).strict(),
...
export async function GET(request:Request){
  try{
    assertAdminHost(request);const actor=await adminActor(request);await limitTenant(`booking-admin-read:${actor.id}`,240);
    ...
    return adminJson(await adminBookingsState(actor,q.tenantId,q.day,q.attention==='1',q.page,q.mode,q.staffId));
  }catch(error){return adminError(error,request);}
}
export async function POST(request:Request){
  try{
    assertAdminHost(request,true);const actor=await adminActor(request);await limitTenant(`booking-admin-write:${actor.id}`,60);
