...
export async function POST(request:Request){
  try{assertAdminHost(request,true);const actor=await adminActor(request);await limitTenant(`booking-policy:${actor.id}`,30);await saveBookingPolicy(actor,await readJson(request));return adminJson({ok:true});}
