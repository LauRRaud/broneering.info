...
import { availableOffers, nextAvailableDay, monthAvailability } from '@/lib/availability';
...
export async function GET(request: Request) {
  try {
    ...
    const parsed=z.object({serviceId:z.uuid(),date:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),staffId:z.uuid().optional(),next:z.literal('1').optional(),month:z.literal('1').optional()}).refine(d=>!(d.next&&d.month)).safeParse(Object.fromEntries(new URL(request.url).searchParams));
    if(!parsed.success) throw new AppError(400,'INVALID_INPUT','Vali teenus ja kuupäev.');
    if(parsed.data.month){
      await limitTenant(`month-client:${tenant.id}:${trustedClientIp(request)}`,30);
      await limitTenant(`month-tenant:${tenant.id}`,300);
      return json(await monthAvailability(tenant,parsed.data.serviceId,parsed.data.date,parsed.data.staffId));
    }
    if(parsed.data.next){
