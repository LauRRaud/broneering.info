...
import StaffPhotoEditor from './admin/staff-photo-editor';
...
type Props={tenantId:string;catalog:ServiceManagementState;busy:boolean;save:Save;onPhotoSaved:()=>Promise<void>};
...
function StaffForm({item,save}:{item?:ManagedStaff;save:Save}){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget,d=new FormData(form);if(await save('save-staff',{id:item?.id,version:item?.version,name:text(d,'name'),title:text(d,'title'),bio:text(d,'bio'),photoUrl:text(d,'photoUrl'),publicPhone:text(d,'publicPhone'),active:d.has('active'),online:d.has('online')})&&!item)form.reset();}
  return <form onSubmit={submit}><Input name="name" label={t("Töötaja nimi")} value={item?.name} max={120}/><label>{t("Amet ")}<input name="title" defaultValue={item?.title} maxLength={150}/></label><label>{t("Avalik telefon (valikuline)")}<input name="publicPhone" type="tel" defaultValue={item?.publicPhone??''} maxLength={30}/></label><p>{t("Selle välja number on avalik broneerimislehel. Lisa ainult number, millele klient võib helistada. Tühjenda väli, et number eemaldada.")}</p><label>{t("Avalik lühitutvustus ")}<textarea name="bio" defaultValue={item?.bio} maxLength={1000}/></label>{item?.photoUrl.startsWith('/api/staff-photos/')?<input type="hidden" name="photoUrl" value={item.photoUrl}/>:<label>{t("Foto HTTPS-aadress (valikuline) ")}<input name="photoUrl" type="url" pattern="https://.*" defaultValue={item?.photoUrl} maxLength={2000}/></label>}{!item&&<p>{t("Faili või kaamerafoto lisamiseks salvesta esmalt töötaja. Seejärel ava tema fotovalikud.")}</p>}<Flags active={item?.active} online={item?.online}/><button>{t("Salvesta töötaja")}</button></form>;
}
function StaffEditor({tenantId,item,save,onSaved}:{tenantId:string;item:ManagedStaff;save:Save;onSaved:()=>Promise<void>}){
  const [dirty,setDirty]=useState(false);
  return <><div onChange={()=>setDirty(true)}><StaffForm item={item} save={save}/></div><StaffPhotoEditor tenantId={tenantId} staff={item} onSaved={onSaved} disabled={dirty}/></>;
}
...
export default function ServiceManagement({tenantId,catalog,busy,save,onPhotoSaved}:Props){
  const {t,locale}=useI18n();
  ...
  {catalog.canEditStructure&&<><h4>{t("Töötajad")}</h4><p>{t("Profiil ei vaja sisselogimiskontot. Siia lisa ainult avalik tutvustus. Graafikuid saad hallata töögraafikute jaotises.")}</p>{catalog.staff.map(st=><details key={st.id}><summary>{st.name}{!st.active?t(" · arhiveeritud"):!st.online?t(" · veebis peidetud"):''}</summary><StaffEditor key={st.version} tenantId={tenantId} item={st} save={run} onSaved={onPhotoSaved}/></details>)}<details><summary>{t("Lisa töötaja")}</summary><StaffForm save={run}/></details></>}
