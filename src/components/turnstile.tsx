...
export default function Turnstile({siteKey,onToken,resetSignal,label}:{siteKey:string;onToken:(token:string)=>void;resetSignal:number;label:string}){
  ...
  const render=()=>{if(element.current&&window.turnstile&&widget.current===undefined)widget.current=window.turnstile.render(element.current,{sitekey:siteKey,action:'booking',appearance:'interaction-only',callback:(token:string)=>tokenCallback.current(token),'expired-callback':()=>tokenCallback.current(''),'error-callback':()=>tokenCallback.current('')});};
  useEffect(()=>{
  ...
  return <><Script src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit" strategy="afterInteractive" onReady={render}/><div ref={element} className={styles.challenge} aria-label={label}/></>;
}
