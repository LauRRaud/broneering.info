...
type Props={tenant:Catalog['tenant'];service:Service;offer:Offer;form:Fields;errors:ContactErrors;locked:boolean;state:'idle'|'submitting'|'uncertain'|'error';error:string;preview:boolean;smsReminder:boolean;onSmsReminder:(value:boolean)=>void;emailReminder:boolean;onReminder:(value:boolean)=>void;onField:(field:keyof Fields,value:string)=>void;onSubmit:(event:FormEvent<HTMLFormElement>)=>void;onEdit:(step:'service'|'staff'|'time')=>void;canEditStaff:boolean};
export default function BookingConfirmation({tenant,service,offer,form,errors,locked,state,error,preview,emailReminder,onReminder,smsReminder,onSmsReminder,onField,onSubmit,onEdit,canEditStaff}:Props){
  ...
  return <form id="booking-form" className={styles.layout} noValidate onSubmit={onSubmit}>
    <section className={styles.contact} aria-label={t('Sinu andmed')}>
      {(['name','email','phone'] as const).map(field=><div className={styles.field} data-field={field} key={field}><label htmlFor={field}>{t({name:'Teenuse saaja nimi',email:'Kontaktisiku e-post',phone:'Telefoninumber'}[field])}{(field!=='phone'||needsPhone)&&<span className={styles.required}> *</span>}</label>
        ...
        {errors[field]&&<span className={styles.fieldErrorDescription} id={field+'-error'}>{t(errors[field]!)}</span>}
      </div>)}
      <p id="contact-help" className={styles.help}>{t('Kasutame neid andmeid ainult broneeringuga seoses.')}{reminderMinutes>0&&<> {reminderAvailable?t('Meeldetuletuse saadame {timing}.',{timing:reminderTiming}):t('Selle aja meeldetuletuse saatmise hetk on möödas ({timing}).',{timing:reminderTiming})}</>}</p>
      {reminderMinutes>0&&<div className={styles.reminders}><BookingReminder checked={emailReminder&&reminderAvailable} disabled={locked||!reminderAvailable} onChange={onReminder}/>{tenant.demo&&<BookingReminder channel="sms" checked={smsReminder&&reminderAvailable} disabled={locked||!reminderAvailable} onChange={onSmsReminder}/>}</div>}
    </section>
