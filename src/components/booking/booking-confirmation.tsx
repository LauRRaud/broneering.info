...
export default function BookingConfirmation({tenant,service,offer,form,errors,locked,state,error,preview,emailReminder,onReminder,smsReminder,onSmsReminder,onField,onSubmit,onEdit,canEditStaff,children}:Props){
  ...
  return <form id="booking-form" className={styles.layout} noValidate onSubmit={onSubmit}>
    <section className={styles.contact} aria-labelledby="contact-title"><h3 id="contact-title">{t('Sinu andmed')}</h3>
      ...
      {(['name','email','phone'] as const).map(field=><div className={styles.field} data-field={field} key={field}><label htmlFor={field}>{t({name:'Teenuse saaja nimi',email:'Kontaktisiku e-post',phone:'Telefoninumber'}[field])}{field==='phone'&&!needsPhone?<span className={styles.optional}>{' '}{t('valikuline')}</span>:<span className={styles.required}> *</span>}</label>
