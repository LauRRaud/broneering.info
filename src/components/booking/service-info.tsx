...
export default function ServiceInfo({name,description,language,translationNotice}:{name:string;description:string;language:string;translationNotice?:string}){
  ...
  return <><Button ref={trigger} type="button" className={styles.toggle} aria-label={`${t('Teenuse lisainfo')}: ${name}`} aria-haspopup="dialog" aria-expanded={open} aria-controls={id} onClick={()=>setOpen(value=>!value)}><Icon name="info" size={18}/></Button>
