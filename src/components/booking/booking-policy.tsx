...
export default function BookingPolicy({cancellationHours}:{cancellationHours:number}){
  ...
  return <>
    <Button ref={trigger} type="button" className={styles.toggle} aria-label={t('Muutmine ja tühistamine')} aria-haspopup="dialog" aria-controls={id} aria-expanded={open} onClick={()=>setOpen(true)}><Icon name="info" size={18}/></Button>
