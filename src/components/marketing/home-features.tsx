...
export default function HomeFeatures({t}:{t:Translate}){
  return <section id="voimalused" className={styles.root} aria-labelledby="features-title"><h2 id="features-title" className={styles.title}>{t('Vähem korraldamist. Rohkem aega.')}</h2>
    <div className={styles.list}>{features.map(([title,description],index)=><article className={styles.feature} key={title}><span className={styles.number} aria-hidden="true">0{index+1}</span><h3 className={styles.subtitle}>{t(title)}</h3><p className={styles.description}>{t(description)}</p></article>)}</div>
  </section>;
}
